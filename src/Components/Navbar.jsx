import React, { useState } from "react";
import { FaBars, FaTimes } from "react-icons/fa";
import { Link } from "react-scroll";
import resume from "../assets/Pavan_Ambulkar_Resume.pdf";

const NavBar = () => {
  const [nav, setNav] = useState(false);

  const links = [
    {
      id: 1,
      link: "home",
    },
    {
      id: 2,
      link: "about",
    },
    {
      id: 3,
      link: "Technology",
    },
    {
      id: 4,
      link: "Skills",
    },
    {
      id: 5,
      link: "Projects",
    },

    {
      id: 6,
      link: "Statistics",
    },
    {
      id: 7,
      link: "Contact",
    },
  ];

  return (
    <div className="flex  justify-between items-center w-full h-20 px-4 text-grey bg-pink-400		 fixed">
      <div>
        <Link to="home" smooth duration={500}>
          <h1 className="text-5xl cursor-pointer font-signature ml-2">Pavan</h1>
        </Link>
      </div>

      <ul className="hidden md:flex">
        {links.map(({ id, link, href, download }) => (
          <li
            key={id}
            className="px-4 cursor-pointer capitalize font-semibold		 text-gray-500 hover:scale-105 duration-200"
          >
            <Link to={link} smooth duration={500}>
              {link}
            </Link>
          </li>
        ))}
      </ul>

      <div
        onClick={() => setNav(!nav)}
        className="cursor-pointer pr-4 z-10 text-gray-500 md:hidden"
      >
        {nav ? <FaTimes size={30} /> : <FaBars size={30} />}
      </div>

      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-black to-gray-800 text-gray-500">
          {links.map(({ id, link }) => (
            <li
              key={id}
              className="px-4 cursor-pointer capitalize py-6 text-4xl"
            >
              <Link
                onClick={() => setNav(!nav)}
                to={link}
                smooth
                duration={500}
              >
                {link}
              </Link>
            </li>
          ))}
        </ul>
      )}
    </div>
  );
};

export default NavBar;
